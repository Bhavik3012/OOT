...
def load_user_bookings(user_id, filename="bookings.csv"):
    bookings = []
    try:
        with open(filename, "r") as file:
            ...
            for row in reader:
                if row["user_id"] == user_id:
                    bookings.append(row)  # Or construct Booking objects if needed
    except FileNotFoundError:
        pass
    return bookings
...
def cancel_booking(user_id, booking_id):
    bookings = []
    try:
        with open('booking_data.csv', 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                bookings.append(row)
    except FileNotFoundError:
    ...
    if not booking_found:
        print(f"No matching booking found for booking_id: {booking_id} or the user_id does not match.")
    
    with open('booking_data.csv', 'w', newline='') as file:
        fieldnames = ['booking_id', 'user_id', 'service_name', 'origin', 'destination', 'booking_date', 'price']
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
...
def print_welcome_message():
    ...
    print("\033[1;33;40m" + " " * 10 + "Welcome to SyS - Secure your Seat" + " " * 10)
    print("\033[1;32;40m" + "="*50)
    print("\033[0m")
        ...
        if user and user.password == pw:
            ...
            while True:
                ...
                if action == '1':
                    ...
                    user_bookings = load_user_bookings(user.user_id)
                    if user_bookings:
                        print("\n--- Your Bookings ---")
                        for b in user_bookings:
                            print(f"{b['booking_id']} | {b['service_name']} | Date: {b['booking_date']}")
                    else:
                        print("You have no bookings yet.")
                    continue
                elif action == '5':
                    booking_id_to_cancel = input("Enter the booking ID you want to cancel: ")
                    cancel_booking(user.user_id, booking_id_to_cancel)
                elif action == '6':
